 ...
 * Output: [[1],[]]
 */
public class BST776SplitBST {
    /** April_2022 crib answer */
    class Solution {
        public TreeNode[] splitBST(TreeNode root, int target) {
            if (root == null) {
                return new TreeNode[] {null, null};
            } else if (root.val <= target) {
                TreeNode[] nodeArr = splitBST(root.right, target);
                root.right = nodeArr[0];
                nodeArr[0] = root;
                return nodeArr;
            } else {
                TreeNode[] nodeArr = splitBST(root.left, target);
                root.left = nodeArr[0];
                nodeArr[0] = root;
                return nodeArr;
            }
